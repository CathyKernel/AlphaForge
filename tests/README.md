...
**Netlify — static demo (zero build).** The dashboard is a read-only research exhibit, so it also works
without any backend: [`netlify.toml`](netlify.toml) publishes `app/static/` and rewrites every `/api/*`
request onto pre-rendered JSON snapshots (generated by `scripts/make_netlify_snapshot.py` through the
real FastAPI stack, 147 files / ~456 KB). Connect the repo on Netlify with an empty build command and
the demo is live — every tab renders identical numbers to the local server. The only differences:
custom-parameter backtests and horizon queries fall back to their pre-rendered defaults (the UI says so),
because static hosting cannot run Python. Regenerate snapshots after changing factors or data:

```bash
PYTHONPATH=src:. python scripts/make_netlify_snapshot.py   # ~6 min, re-renders app/static/api/snap/
```

**Full backend — anywhere Docker runs.** For the live vectorised backtests, deploy the FastAPI server
(the repo ships `Dockerfile` + `docker-compose.yml`, and the bundled 90 MB data cache makes the image
self-contained):

| Platform | Free tier | Notes |
|---|---|---|
| [Render](https://render.com) | yes | Web service from this repo: `pip install -e .[app]`, start `uvicorn app.server:app`; sleeps after 15 min idle (~1 min cold start) |
| [Hugging Face Spaces](https://huggingface.co/spaces) | yes | Docker SDK, CPU basic — fits quant portfolio demos |
| Railway / Fly.io | paid | Smallest paid tiers, always-on |
